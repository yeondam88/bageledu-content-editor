...
export default function BlogPage() {
  ...
  const fetchExistingPosts = async () => {
    ...
    try {
      ...
      showNotification(
        "Failed to load existing posts: " + (error.message || "Unknown error"),
        "error"
      );
    } finally {
  ...
  const loadPostData = (post: any) => {
    ...
    setValue(
      "tags",
      post.tags ? post.tags.replace(/'/g, "").replace(/\[|\]/g, "") : ""
    );
    ...
    if (post.rawContent) {
      if (!titleKo) {
        const titleKoMatch = post.rawContent.match(
          /title:\s*\n\s*ko:\s*['"]([^]*?)['"](?:\s|$)/
        );
        if (titleKoMatch) {
        ...
        if (!titleKo) {
          const koContentMatch = post.rawContent.match(
            /<div class="ko-content"[^>]*>(?:\s*<h1>(.*?)<\/h1>)?\s*([\s\S]*?)<\/div>/
          );
          ...
          if (!titleKo) {
            const koreanTextMatches = post.rawContent.match(
              /[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F\uA960-\uA97F\uD7B0-\uD7FF]+[^<>\n]*[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F\uA960-\uA97F\uD7B0-\uD7FF]/g
            );
            if (koreanTextMatches && koreanTextMatches.length > 0) {
              const potentialTitle = koreanTextMatches.find(
      ...
      if (!excerptEn) {
        const excerptEnMatch = post.rawContent.match(
          /excerpt:\s*\n\s*en:\s*['"]([^]*?)['"](?:\s|$)/
        );
        if (excerptEnMatch) {
      ...
      if (!excerptKo) {
        const excerptKoMatch = post.rawContent.match(
          /excerpt:\s*\n\s*ko:\s*['"]([^]*?)['"](?:\s|$)/
        );
        if (excerptKoMatch) {
          ...
          if (!excerptKo) {
            ...
            if (koreanTextMatches && koreanTextMatches.length > 1) {
              ...
              if (potentialExcerpt) {
                ...
                if (excerptKo.length > 100)
                  excerptKo = excerptKo.substring(0, 100) + "...";
              }
            }
          }
        }
    ...
    showNotification(
      "Post loaded for editing. Generate a preview to update the content."
    );
  };
  ...
  const generatePreview = async () => {
    if (!formValues.title || !formValues.content) {
      showNotification(
        "Please provide both title and content before generating a preview.",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      // Check if we have a file input and files
      ...
      if (!previewImageUrl && imageFile) {
        ...
        try {
          ...
          if (!uploadRes.ok) {
            ...
            if (!uploadJson.url) {
              ...
              if (
                previewImageUrl &&
                previewImageUrl.includes("digitaloceanspaces.com")
              ) {
                ...
                if (
                  !domainPart.match(/[^.]+\.[^.]+\.digitaloceanspaces\.com/)
                ) {
                  // Extract bucket name
          ...
          showNotification(
            `Image upload error: ${uploadError.message || "Unknown error"}`,
            "error"
          );
        ...
        showNotification(
          "No image selected. You will need to upload an image before publishing.",
          "error"
        );
      }
      ...
      if (editMode && bypassOpenAI && selectedPost) {
        ...
        if (!titleKo && selectedPost.rawContent) {
          ...
          const titleKoMatch = selectedPost.rawContent.match(
            /title:\s*\n\s*ko:\s*['"]([^]*?)['"](?:\s|$)/
          );
          if (titleKoMatch) {
            titleKo = titleKoMatch[1].replace(/''/g, "'");
          ...
          if (!titleKo) {
            ...
            const koContentMatch = selectedPost.rawContent.match(
              /<div class="ko-content"[^>]*>(?:\s*<h1>(.*?)<\/h1>)?\s*([\s\S]*?)<\/div>/
            );
            if (koContentMatch && koContentMatch[1]) {
              titleKo = koContentMatch[1].trim();
            ...
            if (!titleKo) {
              const koreanTextMatches = selectedPost.rawContent.match(
                /[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F\uA960-\uA97F\uD7B0-\uD7FF]+[^<>\n]*[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F\uA960-\uA97F\uD7B0-\uD7FF]/g
              );
              if (koreanTextMatches && koreanTextMatches.length > 0) {
                const potentialTitle = koreanTextMatches.find(
        ...
        if ((!excerptEn || !excerptKo) && selectedPost.rawContent) {
          const excerptEnMatch = selectedPost.rawContent.match(
            /excerpt:\s*\n\s*en:\s*['"]([^]*?)['"](?:\s|$)/
          );
          const excerptKoMatch = selectedPost.rawContent.match(
            /excerpt:\s*\n\s*ko:\s*['"]([^]*?)['"](?:\s|$)/
          );
          ...
          if (!excerptKo) {
            ...
            if (!excerptKo) {
              ...
              if (koreanTextMatches && koreanTextMatches.length > 1) {
                ...
                const potentialExcerpt = koreanTextMatches
                  .filter(
                    (text: string) => text !== titleKo && text.length > 15
                  )
                  .sort((a: string, b: string) => b.length - a.length)[0]; // Use the longest match
...
${contentKo}
        ...
        showNotification(
          "Content has been updated. Check the preview tab.",
          "success"
        );
      } else {
        ...
        if (data.parsed) {
          ...
          showNotification(
            "Bilingual content has been generated. Review it in the preview tab before publishing.",
            "success"
          );
        }
      }
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (!uploadedImageUrl && (!data.image || !data.image[0])) {
        throw new Error(
          "Featured image is required. Please upload an image before publishing."
        );
      }
      ...
      if (data.image && data.image[0]) {
        ...
        try {
          ...
          if (!checkImage.ok) {
            throw new Error(
              "The previously uploaded image cannot be accessed. Please try uploading again."
            );
          }
        } catch (error) {
      ...
      if (
        uploadedImageUrl &&
        uploadedImageUrl.includes("digitaloceanspaces.com")
      ) {
      ...
      if (!uploadedImageUrl) {
        throw new Error(
          "No valid image URL available. Please upload an image."
        );
      }
      ...
      const slug =
        editMode && selectedPost
          ...
          : slugify(preview.titleEn || data.title);
      ...
      showNotification(
        `Your blog post has been ${editMode ? "updated" : "published"}.`,
        "success"
      );
    } catch (e: any) {
  ...
  const handleSubmitDirectly = async () => {
    if (preview) {
      ...
      if (showRawMarkdown) {
        ...
        try {
          ...
          const slug =
            editMode && selectedPost
              ? selectedPost.name.replace(".md", "")
              : slugify(preview.titleEn || currentTitle);
          const filePath = `src/content/blog/${slug}.md`;
          ...
          showNotification(
            error.message || "Failed to publish your post",
            "error"
          );
        } finally {
          setLoading(false);
      ...
      showNotification(
        "Please generate a preview first before publishing.",
        "error"
      );
    }
  };
            ...
            <div className="mb-8">
              ...
              >
                {existingPosts.map((post) => (
                  <Card
                    key={post.path}
                    className={`border border-gray-200 shadow-sm hover:shadow transition-shadow ${
                      existingPosts.length > 3
                        ? "flex-shrink-0 w-full sm:w-[calc(50%-0.5rem)] lg:w-[calc(33.333%-0.667rem)] snap-start"
                        : ""
                    } flex flex-col`}
                  >
                    <div className="relative aspect-[16/10] overflow-hidden rounded-t-md bg-gray-100">
                      {post.image ? (
                        ...
                        <img
                          src="https://bageledu.sfo3.cdn.digitaloceanspaces.com/bageledu-og-image-v3.webp"
                          alt="Bagel Edu"
                          className="object-cover w-full h-full"
                        />
                      )}
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4">
                      ...
                      {editMode && (
                        <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 rounded-md text-sm">
                          ...
                          <span>
                            Editing{" "}
                            {activeLanguage === "en" ? "English" : "Korean"}{" "}
                            version
                          </span>
                        </div>
                      )}
                        ...
                        {loading ? (
                          ...
                          <>
                            <svg
                              ...
                              strokeLinejoin="round"
                            >
                              <path d="M20 11.08V8l-6-6H6a2 2 0 0 0-2 2v16c0 1.1.9 2 2 2h12a2 2 0 0 0 2-2v-3.08c.58-.77 1-1.73 1-2.92 0-1.2-.42-2.15-1-2.92z"></path>
                              <path d="M14 2v6h6"></path>
                              <path d="M22.66 15.5c0 1.8-1.77 3.5-4 3.5h-5.33c-2.23 0-4-1.7-4-3.5s1.77-3.5 4-3.5h5.33c2.23 0 4 1.7 4 3.5z"></path>
                            </svg>
                              ...
                              onClick={() => {
                                ...
                                if (editingMarkdown !== rawMarkdown) {
                                  ...
                                  try {
                                    ...
                                    const enContentMatch =
                                      editingMarkdown.match(
                                        /<div class="en-content"[^>]*>([\s\S]*?)<\/div>/
                                      );
                                    const koContentMatch =
                                      editingMarkdown.match(
                                        /<div class="ko-content"[^>]*>([\s\S]*?)<\/div>/
                                      );

                                    const titleEnMatch = editingMarkdown.match(
                                      /title:\s*\n\s*en:\s*['"]([^'"]*)['"]/
                                    );
                                    const titleKoMatch = editingMarkdown.match(
                                      /title:\s*\n\s*ko:\s*['"]([^'"]*)['"]/
                                    );

                                    const excerptEnMatch =
                                      editingMarkdown.match(
                                        /excerpt:\s*\n\s*en:\s*['"]([^'"]*)['"]/
                                      );
                                    const excerptKoMatch =
                                      editingMarkdown.match(
                                        /excerpt:\s*\n\s*ko:\s*['"]([^'"]*)['"]/
                                      );
                                    ...
                                    if (
                                      ...
                                      if (titleEnMatch && titleEnMatch[1])
                                        updatedPreview.titleEn =
                                          titleEnMatch[1];
                                      if (titleKoMatch && titleKoMatch[1])
                                        updatedPreview.titleKo =
                                          titleKoMatch[1];
                                      if (excerptEnMatch && excerptEnMatch[1])
                                        updatedPreview.excerptEn =
                                          excerptEnMatch[1];
                                      if (excerptKoMatch && excerptKoMatch[1])
                                        updatedPreview.excerptKo =
                                          excerptKoMatch[1];
            ...
            <span className="flex items-center gap-1">
              ...
              >
                <svg
                  ...
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className="animate-pulse transition-transform duration-300 hover:scale-110 scale-105"
                >
