...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (parsed) {
      const { titleEn, titleKo, excerptEn, excerptKo, contentEn, contentKo } =
        parsed;
      ...
      if (
        formattedImageUrl &&
        formattedImageUrl.includes("digitaloceanspaces.com")
      ) {
...
${contentKo ? `${contentKo.replace(/\{\s*'\s*\}\s*\n*/g, "\n")}` : ""}
    ...
    if (prompt && type === "blog_ideas") {
      ...
      const titleEnMatch = generatedContent.match(
        /TITLE_EN:([\s\S]+?)(?=TITLE_KO:|$)/
      );
      const titleKoMatch = generatedContent.match(
        /TITLE_KO:([\s\S]+?)(?=EXCERPT_EN:|$)/
      );
      const excerptEnMatch = generatedContent.match(
        /EXCERPT_EN:([\s\S]+?)(?=EXCERPT_KO:|$)/
      );
      const excerptKoMatch = generatedContent.match(
        /EXCERPT_KO:([\s\S]+?)(?=CONTENT_EN:|$)/
      );
      const contentEnMatch = generatedContent.match(
        /CONTENT_EN:([\s\S]+?)(?=CONTENT_KO:|$)/
      );
      const contentKoMatch = generatedContent.match(
        /CONTENT_KO:([\s\S]+?)(?=$)/
      );
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a language detection assistant. Identify the primary language of the text as either 'English' or 'Korean'. Respond with ONLY 'English' or 'Korean'.",
        },
    ...
    const detectedLanguage =
      detectResponse.choices[0].message.content?.trim() || "English";
    const isKorean = detectedLanguage === "Korean";

    // Create a comprehensive prompt for the AI to generate content in both languages
    const promptMessage = `
    I need a bilingual blog post based on this ${
      isKorean ? "Korean" : "English"
    } content. Please enhance and translate it to create content in BOTH languages.
    ...
    if (frontMatterMatch) {
      ...
      const titleEnMatch = frontMatter.match(
        /title:\s*\n\s*en:\s*['"]([^'"]*)['"]/
      );
      const titleKoMatch = frontMatter.match(
        /title:\s*\n\s*ko:\s*['"]([^'"]*)['"]/
      );
      const excerptEnMatch = frontMatter.match(
        /excerpt:\s*\n\s*en:\s*['"]([^'"]*)['"]/
      );
      const excerptKoMatch = frontMatter.match(
        /excerpt:\s*\n\s*ko:\s*['"]([^'"]*)['"]/
      );

      // Extract English and Korean content sections
      const enContentMatch = contentSection.match(
        /<div class="en-content"[^>]*>([\s\S]*?)<\/div>/
      );
      const koContentMatch = contentSection.match(
      ...
      const titleEnAltMatch = generatedContent.match(
        /title:\s*\n\s*en:\s*['"]([^'"]*)['"]/
      );
      const titleKoAltMatch = generatedContent.match(
        /title:\s*\n\s*ko:\s*['"]([^'"]*)['"]/
      );

      // Try alternative title patterns
      if (!titleEnAltMatch) {
        const altTitleEnMatch = generatedContent.match(
          /TITLE_EN:?\s*(.*?)(?=TITLE_KO|EXCERPT|$)/i
        );
      ...
      if (!titleKoAltMatch) {
        const altTitleKoMatch = generatedContent.match(
          /TITLE_KO:?\s*(.*?)(?=EXCERPT|CONTENT|$)/i
        );
      ...
      const excerptEnAltMatch = generatedContent.match(
        /EXCERPT_EN:?\s*(.*?)(?=EXCERPT_KO|CONTENT|$)/i
      );
      const excerptKoAltMatch = generatedContent.match(
        /EXCERPT_KO:?\s*(.*?)(?=CONTENT|$)/i
      );
      ...
      const contentEnAltMatch = generatedContent.match(
        /CONTENT_EN:?\s*([\s\S]*?)(?=CONTENT_KO|$)/
      );
      const contentKoAltMatch = generatedContent.match(
        /CONTENT_KO:?\s*([\s\S]*?)$/
      );
      ...
      if (!contentEn.includes("<p>")) {
        const enDivMatch = generatedContent.match(
          /<div class="en-content"[^>]*>([\s\S]*?)<\/div>/
        );
      ...
      if (!contentKo.includes("<p>")) {
        const koDivMatch = generatedContent.match(
          /<div class="ko-content"[^>]*>([\s\S]*?)<\/div>/
        );
    ...
    if (!titleKo) {
      ...
      if (titleEn && titleEn !== "English Title") {
        try {
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a Korean translator. Translate the given English title to natural, fluent Korean. Respond ONLY with the Korean translation, nothing else.",
              },
              {
                role: "user",
          ...
          const generatedKoreanTitle =
            koreanTitleResponse.choices[0].message.content?.trim();
    ...
    if (!excerptKo || excerptKo === "한국어 발췌문.") {
      ...
      if (excerptEn && excerptEn !== "English excerpt for the blog post.") {
        try {
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a Korean translator. Translate the given English excerpt to natural, fluent Korean. Respond ONLY with the Korean translation, nothing else.",
              },
              {
                role: "user",
          ...
          const generatedKoreanExcerpt =
            koreanExcerptResponse.choices[0].message.content?.trim();
          if (
            generatedKoreanExcerpt &&
            generatedKoreanExcerpt !== "한국어 발췌문."
          ) {
...
<p>
  ${
    contentKo.trim().replace(/<p>/g, "").replace(/<\/p>/g, "").trim() ||
    "대학 지원 과정은 학생의 미래를 결정하는 중요한 단계입니다. 이 과정을 통해 학생들은 자신의 관심사와 목표에 맞는 대학을 찾을 수 있습니다."
  }
</p>
