    ...
    const posts = await Promise.all(
      markdownFiles.map(async (file: any) => {
        try {
          ...
          if (!fileResp.ok) {
            console.error(
              `Failed to fetch content for ${file.name}: ${fileResp.status}`
            );
            return null;
          }
          ...
          let excerptEn = excerptEnMatch
            ? excerptEnMatch[1].replace(/''/g, "'")
            : "";
          let excerptKo = excerptKoMatch
            ? excerptKoMatch[1].replace(/''/g, "'")
            : "";

          // IMPROVED: Try alternative regex patterns if the first ones fail
          if (!titleKo) {
            const altTitleKoMatch = frontMatter.match(
              /title:[\s\S]*?ko:[\s\S]*?['"]([^'"]*(?:['"][^'"]*)*?)['"](?:\s|$)/
            );
            if (altTitleKoMatch) {
          ...
          if (!excerptKo) {
            const altExcerptKoMatch = frontMatter.match(
              /excerpt:[\s\S]*?ko:[\s\S]*?['"]([^'"]*(?:['"][^'"]*)*?)['"](?:\s|$)/
            );
          ...
          if (!titleKo || !excerptKo) {
            ...
            const koContentMatch = content.match(
          ...
          if (!titleKo || !excerptKo) {
            ...
            const koreanTextMatches = content.match(
              /[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F\uA960-\uA97F\uD7B0-\uD7FF]+[^<>\n]*[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F\uA960-\uA97F\uD7B0-\uD7FF]/g
            );
          ...
          const enContentMatch = content.match(
            /<div class="en-content"[^>]*>\s*(?:<h1>.*?<\/h1>)?\s*([\s\S]*?)<\/div>/
          );
          const koContentMatch = content.match(
            /<div class="ko-content"[^>]*>\s*(?:<h1>.*?<\/h1>)?\s*([\s\S]*?)<\/div>/
          );
